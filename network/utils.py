...
def get_im2col_indices(x_shape, filter_height, filter_width, padding=0, stride=1):
    # Function for getting current dimensions
    N, C, H, W = x_shape
    ...
    j0 = np.tile(np.arange(filter_width), filter_height * C)  # similarly i0

    j1 = stride * np.tile(np.arange(out_width), out_height)  # similarly i1
...
def getWindows(input_, output_size, filter_size, padding=0, stride=1, dilate=0):
    ...
    if dilate != 0:  # insert zeros in input_ between his elements
    ...
    # count of bytes for getting to elements from dimensions
    ...
    # count of bytes that necessary to get correct tensor
